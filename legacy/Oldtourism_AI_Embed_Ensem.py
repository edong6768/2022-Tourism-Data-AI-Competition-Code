...
from TourClassification import TourClassification1EmbedEnsemble as TourClassification
...
def train(net, train_data, test_data, device):
    ...
    for epoch in range(epochs):
        ...
        for img, text, label in pbar:
            ...
            pred, predtxt, predimg = net(img, text)
            L, Lt, Li = loss(pred, label), loss(predtxt, label), loss(predimg, label)
            ...
            tot_loss += ((L+Lt+Li)/3).cpu().detach().item()
            
            l_idx = torch.argmax(label.cpu(), dim=1)
            p_idx = torch.argmax((0.25*predtxt+0.25*predimg+0.5*pred).cpu(), dim=1).detach()
            acc+=torch.sum(l_idx==p_idx).item()
            pbar.set_description(f'Epoch: {epoch+1}/{epochs}, correct:{acc}/{len(train_data)}, loss:{((Lt+Li)/2).detach().item()}')
        ...
        if best_loss > tot_loss:
            ...
            torch.save(best_state, './ckpt/bestsaved-model-ens.pt')
...
def inference(net, classes, device):
    net.load_state_dict(torch.load('./ckpt/bestsaved-model-ens.pt'))
